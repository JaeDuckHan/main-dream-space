    ...
    <div className="bg-[#E3F2FD] h-[44px] flex items-center justify-center px-4 text-[14px]">
      <div className="max-w-[1100px] w-full flex items-center justify-between">
        <div className="flex items-center gap-2 text-foreground">
          <span>🏖️</span>
          <span className="font-semibold">{data.city} 한달살기</span>
          <span className="text-[#AAA]">·</span>
          <span>{format(start, "M/d")}–{format(end, "M/d")}</span>
          <span className="text-[#AAA]">·</span>
          <span className="font-bold">D{dDay > 0 ? `-${dDay}` : dDay === 0 ? "-Day" : `+${Math.abs(dDay)}`}</span>
          <span className="text-[#AAA]">·</span>
          <span>준비 {pct}%</span>
          <span className="text-[#AAA]">·</span>
          <button
            onClick={() => navigate("/planner")}
            className="text-[hsl(214,100%,40%)] font-semibold hover:underline"
          >
            보기 →
          </button>
        </div>
        <button onClick={() => setHidden(true)} className="text-[#AAA] hover:text-foreground ml-4">
          <X size={16} />
        </button>
      </div>
    </div>
